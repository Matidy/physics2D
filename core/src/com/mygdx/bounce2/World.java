...
public class World {
	...
	public Vector2[] getRelevantBallPoints (Wall wall, Ball ball) {
		...
		if (wall.quadrant == 2 || wall.quadrant == 4) { // quadrants are +/- 90 as the ball point is found from the normal of the wall
			x = (float) (Math.sin(angle) * ball.getRadius());
			y = (float) (Math.cos(angle) * ball.getRadius()); 
		} 
		else {
			x = (float) (Math.cos(angle) * ball.getRadius());
			y = (float) (Math.sin(angle) * ball.getRadius());
		}
